            ...
            public void onSuccess(List<Comment> list) {
                if (list != null) {
                    for (Comment comment : list) {
                        ...
                        lists.setCover(comment.getBooklist().getCover());
                        lists.setBookname(comment.getBooklist().getName());
                        lists.setAuthor(comment.getBooklist().getAuthor());
            ...
            public void onSuccess(List<Ground> list) {
                if (list != null) {
                    for (Ground ground : list) {
                        ...
                        if (ground.getBooklist() != null) {
                            lists.setCover(ground.getBooklist().getCover());
                            lists.setBookname(ground.getBooklist().getName());
                            lists.setAuthor(ground.getBooklist().getAuthor());
                        }
                        System.out.println("time----" + lists.getTime());
                        latestLists.add(lists);
                    }
...
//
        ...
        query3.addWhereExists("booklist");
            ...
            public void onSuccess(List<Likes> list) {
                if (list != null) {
                    for (Likes likes : list) {
                        ...
                        lists.setCover(likes.getBooklist().getCover());
                        lists.setBookname(likes.getBooklist().getName());
                        lists.setAuthor(likes.getBooklist().getAuthor());
                        latestLists.add(lists);
                    }
                    Collections.sort(latestLists, new sortByTime());
                    switch (latestLists.size()){
